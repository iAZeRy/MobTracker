...
function showInfoPanel(mob) {
    openModal(`<h2>${mob.name}</h2><p>${mob.description || "Keine Beschreibung."}</h2></p>${mob.spawnrate || "Keine Spawnrate."}</h3></p>${mob.lebenspunkte || "Keine Lebenspunkte."}</h3></p>${mob.biom || "Kein Biom."}</p>`);
}
